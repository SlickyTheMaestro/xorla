...
function AuthScreen({ onDone }) {
  ...
  if (step === 'role') {
    return wrap("Who's opening Xorla?", 'This decides what you\'ll see next.', (
      <>
        <button onClick={() => setStep('owner')} className="w-full rounded-xl p-4 mb-2.5 text-left" style={{ background: C.surfaceRaised, border: `1px solid ${C.line}` }}>
        ...
        <button onClick={() => setStep('staff')} className="w-full rounded-xl p-4 text-left" style={{ background: C.surfaceRaised, border: `1px solid ${C.line}` }}>
  ...
  if (step === 'code') {
    return wrap('Your business code', "Share this with your staff — it's how they join your business, not a password.", (
      <>
        ...
        <button onClick={() => onDone()} className="w-full rounded-xl py-3 text-[13.5px] font-semibold" style={{ background: C.sage, color: C.bg }}>Continue to my dashboard</button>
      </>
    ));
  }
  ...
  if (step === 'staff') {
    ...
    return wrap('Join your business', mode === 'login' ? 'Log back in.' : "Enter your employer's business code to join.", (
      <>
        ...
        <button disabled={loading || !form.email || !form.password || (mode === 'signup' && (!form.code || !form.name))} onClick={submitStaff} className="w-full rounded-xl py-3 text-[13.5px] font-semibold mb-4" style={{ background: C.sage, color: C.bg, opacity: loading ? 0.6 : 1 }}>
  ...
  return wrap(mode === 'login' ? 'Welcome back' : 'Create your account', mode === 'login' ? 'Log in to see how your business is doing.' : 'Takes less than a minute to get started.', (
    <>
      ...
      <button disabled={loading || !form.email || !form.password || (mode === 'signup' && !form.business)} onClick={submitOwner} className="w-full rounded-xl py-3 text-[13.5px] font-semibold mb-4" style={{ background: C.sage, color: C.bg, opacity: loading ? 0.6 : 1 }}>
